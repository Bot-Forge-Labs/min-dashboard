...
export function ModerationTable() {
  ...
  const fetchModLogs = async () => {
    try {
      ...
      // Declare variables here so they can be reassigned
      ...
      // If view doesn't exist, try regular table
      ...
      const uniqueGuilds = Array.from(
        new Set((data || []).map((log) => log.guild_id))
      ).map((guildId) => ({
        id: guildId,
  ...
  const filteredLogs = modLogs.filter((log) => {
    const userUsername = log.user_username ?? "";
    const moderatorUsername = log.moderator_username ?? "";
    ...
    const matchesSearch =
      userUsername.toLowerCase().includes(lowerSearch) ||
      moderatorUsername.toLowerCase().includes(lowerSearch) ||
          ...
          <Table>
            ...
            <TableBody>
              {filteredLogs.map((log) => {
                const userUsername = log.user_username ?? "Unknown User";
                const userId = log.user_id ?? "Unknown ID";
                const action = log.action ?? "unknown";
                const moderatorUsername =
                  log.moderator_username ?? "Unknown Moderator";
                const moderatorId = log.moderator_id ?? "Unknown ID";
                const guildName =
                  guilds.find((g) => g.id === log.guild_id)?.name ||
                  `Guild ${log.guild_id?.slice(-4) ?? "N/A"}`;
