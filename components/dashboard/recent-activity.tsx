  ...
  return (
    <Card className="bg-white/5 backdrop-blur-xl border border-emerald-400/20 shadow-xl">
      <CardHeader>
        ...
        <CardDescription className="text-emerald-200/80">Latest moderation actions across all servers</CardDescription>
      </CardHeader>
            ...
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="outline" className={getTypeColor(activity.action)}>
                    {activity.action}
                  </Badge>
                </div>
                <div className="text-sm text-emerald-200/70">
                  {activity.user_username || activity.user_id}
                  {activity.moderator_username && ` by ${activity.moderator_username}`}
                </div>
              </div>
